...
def get_config_dir():
    ...
    if sys.platform == 'win32':
        config_dir = os.path.join(os.environ.get('APPDATA', ''), 'Text2LLM')
    elif sys.platform == 'darwin':
        config_dir = os.path.expanduser('~/Library/Application Support/Text2LLM')
    else:
        config_dir = os.path.expanduser('~/.config/Text2LLM')
...
def launch_main_app():
    ...
    try:
        ...
        messagebox.showerror(
            "Launch Error",
            f"An error occurred launching Text2LLM:\n\n{e}"
        )
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        if is_setup_complete():
            ...
            if ollama_ok and models_ok:
                ...
                response = messagebox.askyesno(
                    ...
                    "Note: Text2LLM requires llama3.2:3b model."
                )
                ...
                if response:
                    ...
                    messagebox.showinfo(
                        "Launching Anyway",
                        "Text2LLM will try to launch, but some features may not work.\n\n"
        ...
        messagebox.showerror(
            "Startup Error",
            f"Text2LLM encountered an error during startup:\n\n{e}\n\n"
